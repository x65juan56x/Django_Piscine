...
def display(request):
    try:
        ...
        return render(request, 'ex04/display.html', {'movies': movies_data})
...
def remove(request):
    try:
        ...
        if request.method == 'POST':
            movie_to_remove = request.POST.get('movie_dropdown')

            cur.execute("DELETE FROM ex04_movies WHERE episode_nb = %s;", [movie_to_remove])
            conn.commit()

        cur.execute("SELECT episode_nb, title FROM ex04_movies;")
        movies_data = cur.fetchall()
        ...
        return render(request, 'ex04/remove.html', {'movies': movies_data})
